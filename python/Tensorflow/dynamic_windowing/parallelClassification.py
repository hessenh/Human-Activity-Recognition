...
class CNN_TEST(object):
	...
	def __init__(self, network_type, index, models):
		...
		df_results = pd.DataFrame(index = np.arange(len(raw_signal_labels)) ,columns=np.arange(len(models)*17+1))
		for i in range(0,len(raw_signal_labels)):
			df_results[17*len(models)][i] = raw_signal_labels[i];
...
#HENT MODELLENE
models = [["original",300,20,40,200,"SAME","0.5"],["original",450,20,40,200,"SAME","0.75"], ["original",600,20,40,200,"SAME","1.0"],["original",750,20,40,200,"SAME","1.25"],["original",900,20,40,200,"SAME","1.5"]]
